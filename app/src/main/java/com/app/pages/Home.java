...
public class Home extends VerticalLayout {
    public Home(User user, App parent) {
        ...
        notification.open();
        add(new H1("Welcome " + user.getFirstName()), new MessageInput(submitEvent -> {
        }));
        try {
            for (Announcement iter : parent.getAnnouncementService().getAnnouncements()) {
                add(new PostLayout(parent.getUserService().findUserById(iter.getFrom()), user, iter, parent.getAnnouncementService(), true));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
